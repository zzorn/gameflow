...
object Logging {
   ...
   */
  def initializeLogging(loggingRootPackages: String *) {
    val properties: Properties = new Properties()

    // Set logging levels and appenders
    properties.setProperty("log4j.rootLogger", "DEBUG, CONSOLE_APPENDER")
    loggingRootPackages foreach {p =>
      properties.setProperty(p, "DEBUG, CONSOLE_APPENDER")
    }
...
/**
 * A mixin that provides various logging methods.
 */
trait Logging {
